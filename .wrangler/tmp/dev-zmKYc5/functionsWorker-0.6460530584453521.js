...
// .wrangler/tmp/pages-mG3lT5/functionsWorker-0.6460530584453521.mjs
...
function splitLocaleFromPath(pathname) {
  const match2 = pathname.match(/^\/([a-z]{2})(\/.*|)$/);
  if (match2 && ENABLED_LOCALES.has(match2[1])) {
    return { locale: match2[1], rest: match2[2] || "/" };
  }
  return { locale: null, rest: pathname };
}
__name(splitLocaleFromPath, "splitLocaleFromPath");
__name2(splitLocaleFromPath, "splitLocaleFromPath");
...
function matchAcceptLanguage(header) {
  ...
  for (const { lang } of entries) {
    if (ENABLED_LOCALES2.has(lang)) return lang;
    const base = lang.split("-")[0];
    if (ENABLED_LOCALES2.has(base)) return base;
  }
  return null;
}
__name(matchAcceptLanguage, "matchAcceptLanguage");
__name2(matchAcceptLanguage, "matchAcceptLanguage");
function getCookie2(cookieHeader, name) {
  if (!cookieHeader) return null;
  const match2 = cookieHeader.match(new RegExp(`(?:^|;\\s*)${name}=([^;]*)`));
  return match2 ? decodeURIComponent(match2[1]) : null;
}
__name(getCookie2, "getCookie2");
__name2(getCookie2, "getCookie");
var onRequest2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const cookieHeader = request.headers.get("cookie");
  const savedLocale = getCookie2(cookieHeader, "locale-pref");
  if (savedLocale && ENABLED_LOCALES2.has(savedLocale)) {
...
// .wrangler/tmp/bundle-VMVYnB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VMVYnB/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6460530584453521.js.map
